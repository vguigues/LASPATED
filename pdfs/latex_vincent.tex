...
\Plainauthor{Vincent Guigues, Anton J. Kleywegt, Giovanni Amorim, André M. Krauss, Victor Hugo Nascimento} %% comma-separated
...
\Abstract{
  We describe methods, tools, and a software library called LASPATED, available on GitHub\footnote{at https://github.com/vguigues/} to fit models using spatio-temporal data and space-time discretization.
...
We describe a \proglang{Python} package to perform various types of space and time discretization.
We also describe two packages for the calibration of the models, one in \proglang{MATLAB} and one in \proglang{C++}.
...
\centering
\begin{tabular}{c}
\includegraphics{disc_10x10.pdf}
\end{tabular}
...
\centering
\begin{tabular}{c}
\includegraphics{disc_100x100.pdf}
\end{tabular}
...
\subsection[Matlab LASPATED calibration functions]{\proglang{MATLAB} LASPATED calibration functions}
...
\subsubsection[Matlab examples]{\proglang{MATLAB} examples}
...
LASPATED provides \proglang{MATLAB} and \proglang{C++} functions for the calibration of models as in Section~\ref{sec:model1} for given values of weights $W_{G}$ and $w_{i,j}$, by solving
...
LASPATED also provides \proglang{MATLAB} and \proglang{C++} cross validation functions for the selection of weights $W_{G}$ and $w_{i,j}$ and the computation of the corresponding optimal intensities
$\lambda_{c.i.t}$ that solve problem~\eqref{eqn:reformmodel1}
for the selected weights.

LASPATED also provides \proglang{MATLAB} and \proglang{C++} functions for the
...
\centering
\begin{tabular}{cc}
\includegraphics[scale=0.3]{lambdaconstanteven.pdf} &
\includegraphics[scale=0.3]{lambdaconstantodd.pdf} \\
...
\centering
\begin{tabular}{cc}
\includegraphics[scale=0.1]{2weeks_Color_Unknown_v3.pdf} &
\includegraphics[scale=0.1]{20weeks_Color_Unknown_v3.pdf} \\
$\hspace*{0.6cm}$ $N_{i,t} = 14$ periods observed & $\hspace*{0.6cm}$ $N_{i,t} = 140$ periods observed
\vspace{5mm} \\
\includegraphics[scale=0.1]{100weeks_Color_Unknown_v3.pdf} &
\includegraphics[scale=0.1]{1000weeks_Color_Unknown_v3.pdf}\\ 
...
\centering
\begin{tabular}{cc}
\includegraphics[scale=0.1]{rates2site1v3.pdf} &
\includegraphics[scale=0.1]{rates2site6v3.pdf} \\
$\hspace*{0.6cm}$ $N_{i,t} = 14$, zone $1 \in \mathcal{I}_{\mathcal{R}}$ & $\hspace*{0.6cm}$ $N_{i,t} = 14
$, zone $6 \in \mathcal{I}_{\mathcal{B}}$
\vspace{5mm} \\
\includegraphics[scale=0.1]{rates20site1v3.pdf}&
\includegraphics[scale=0.1]{rates20site6v3.pdf}\\
...
\centering
\begin{tabular}{cc}
\includegraphics[scale=0.01]{2weeks_Color_Unknown_v3_model2.pdf} &
\includegraphics[scale=0.01]{20weeks_Color_Unknown_v3_model2.pdf} \\
$\hspace*{0.6cm}$ $N_{i,t} = 14$ periods observed & $\hspace*{0.6cm}$ $N_{i,t} = 140$ periods observed
\vspace{5mm} \\
\includegraphics[scale=0.01]{100weeks_Color_Unknown_v3_model2.pdf} &
\includegraphics[scale=0.01]{1000weeks_Color_Unknown_v3_model2.pdf} \\
...
\proglang{C++} calibration module contains two classes: GeneratorRegressor, to be used when {\tt{model=reg}} and GeneratorNoRegressor, when {\tt{model=noreg}}. The set of attributes on both classes is similar to \proglang{MATLAB} functions:
\begin{itemize}
...
\subsubsection{Examples}
 ...
 \proglang{C++} calibration functions can also be run from the command line. The outputs are written in files. The following two examples are available.

\begin{itemize}
    \item Examples with simulated data. \proglang{C++} code corresponding to 
    \proglang{MATLAB} \textbf{laspatedex1noreg} function can be run with
    $$
    {\tt{\$./laspated\text{ }--model=no\_reg}}
    $$
    while 
    \proglang{MATLAB} \textbf{laspatedex1reg} function can be run with
    $$
    ...
    corresponding to \proglang{MATLAB} function 
