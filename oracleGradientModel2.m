...
function [gradient]=oracleGradientModel2(nbObservations,nbArrivals,x,C,D,T,R,regressor,nbLandTypes,sizex,Groups,whichgroup,weight,durations,indexBeta)
        ...
        for d=1:D
            for t=1:T
                for m=1:length(Groups{1,whichgroup(d,t)})
                    ...
                    if ((d1~=d)||(t1~=t))
                        for j=1:(1+nbLandTypes)
                            gradient(indexBeta(c,d,t,j))=gradient(indexBeta(c,d,t,j))+(2*weight/durations(t))*((gradient(indexBeta(c,d,t,j))/durations(t))-(gradient(indexBeta(c,d1,t1,j))/durations(t1)));
                        end
                    end
                end
            end
