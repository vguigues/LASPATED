...
for i=8:15
    aux.d=i;
    aux.t=1;
    Groups{1,5}=[Groups{1,5};aux];
end
Groups{1,6}=[];
for i=8:15
    aux.d=i;
    aux.t=2;
    Groups{1,6}=[Groups{1,6};aux];
end
Groups{1,7}=[];
for i=8:15
    aux.d=i;
    aux.t=3;
    Groups{1,7}=[Groups{1,7};aux];
end
Groups{1,8}=[];
for i=8:15
    aux.d=i;
    aux.t=4;
    Groups{1,8}=[Groups{1,8};aux];
end
nbWeeks=52*150;
...
epsilon=10^(-6);
beta=2*epsilon*ones(C*D*T*(nbLandTypes+1),1);
sigma=0.5;
betaBar=1;
iterMax=100;
weight=1;
...
indexBetaA=zeros(C,D,T,1+nbLandTypes);
for gnb=1:length(Groups)
    for cnb=1:C
        sizex=0;
        nbC=length(Groups{1,gnb})*R;
        for m=1:length(Groups{1,gnb})
            for j=1:(1+nbLandTypes)
                sizex=sizex+1;
                indexBetaA(cnb,Groups{1,gnb}(m).d,Groups{1,gnb}(m).t,j)=sizex;
            end
        end
        betaA=2*epsilon*ones(sizex,1);
        lbounds=zeros(sizex,1);
        [xred,fVal]=projectedGradientArmijoFeasible2lm(nbObservations,nbCalls,betaA,C,D,T,R,weight,regressor,nbLandTypes,iterMax,sigma,epsilon,indexBetaA,Groups,durations,sizex,whichgroup,lbounds,gnb,cnb,nbC);
        for m=1:length(Groups{1,gnb})
            for j=1:(1+nbLandTypes)
                beta(indexBeta(cnb,Groups{1,gnb}(m).d,Groups{1,gnb}(m).t,j))=xred(indexBetaA(cnb,Groups{1,gnb}(m).d,Groups{1,gnb}(m).t,j));
            end
        end
    end
end
    ...
    for t=1:T
        for i=1:R
            for c=1:C
                ...
                rateest=0;
                for j=1:(1+nbLandTypes)
                    rate=rate+betaTeoricos(c,d,t,j)*regressor(j,i);
                    rateest=rateest+beta(indexBeta(c,d,t,j))*regressor(j,i);
                    %if (betaTeoricos(c,d,t,j)>0)
                    %   meanrateBeta=meanrateBeta+100*(abs(x(indexBeta(c,d,t,j))-betaTeoricos(c,d,t,j))/betaTeoricos(c,d,t,j));
                    %   cp=cp+1;
                    %end
                end
                rate
