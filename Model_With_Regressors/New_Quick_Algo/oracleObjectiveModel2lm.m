...
function [f]=oracleObjectiveModel2lm(nbObservations,nbCalls,x,C,D,T,R,regressor,nbLandTypes,Groups,weight,durations,indexBetaA,gnb,cnb)
...
for m=1:length(Groups{1,gnb})
    ...
    for i=1:R
        ...
        for j=1:(1+nbLandTypes)
            rates=rates+x(indexBetaA(cnb,thiday,thistime,j))*regressor(j,i);
        end
    ...
    for index1=1:length(Groups{1,gnb})
        ...
        for index2=1:length(Groups{1,gnb})
            if (index2~=index1)
                ...
                for j=1:(1+nbLandTypes)
                    f=f+(weight/2)*((x(indexBetaA(cnb,d1,t1,j))/durations(t1))-(x(indexBetaA(cnb,d2,t2,j))/durations(t2)))^2;
                end
            end
        end
    end
