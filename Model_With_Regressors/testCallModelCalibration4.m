...
model='reg';
[cputime,weight,lambda]=crossValidation(method,sample,T,R,P,sigma,sigma,x,iterMax,proportion,epsilon,durations,Groups,whichgroup,weights,params);

[cputime,weight,x]=crossValidationReg(sample,indexBeta,x,C,D,T,R,regressor,nbLandTypes,proportion,betaBar,sizex,iterMax,sigma,epsilon,nbWeeks*7,isHolidays,obsbefore,Groups,durations,sizex,whichgroup,weights);

%First projected gradient
[x,fVal]=projectedGradientArmijoFeasible2(nbObservations,nbArrivals,beta,C,D,T,R,weight,regressor,nbLandTypes,iterMax,sigma,epsilon,indexBeta,Groups,durations,sizex,whichgroup);
