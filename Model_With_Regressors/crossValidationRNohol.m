...
function [weight,x]=crossValidationRNohol(sampleCalls,indexBeta,x,C,D,T,R,regressor,nbLandTypes,proportion,betaBar,sizex,iterMax,sigma,epsilon,nbObs)

weights=[0.01,0.1,1,10,100,1000,10^6,10^7];
maxLikelihood=inf;
...
for indexweights=1:length(weights)
    ...
    for indexCross=1:floor((1/proportion))
                ...
                for d=1:D
                    for t=1:T
                        for i=1:R
                            ...
                            nbCallsCurrent(c,day,t,i)=nbCallsCurrent(c,day,t,i)+sampleCalls{c,day,t,i}(index);      
                        end
                    end
                end
            end
        ...
        [x,fVal]=projectedGradientArmijoFeasible2(nbObservationsCurrent,nbCallsCurrent,beta,C,D,T,R,weights(indexweights),regressor,nbLandTypes,iterMax,sigma,epsilon,indexBeta);
                ...
                for d=1:D
                    for t=1:T
                        for i=1:R
                            ...
                            nbCallsRemaining(c,day,t,i)=nbCallsRemaining(c,day,t,i)+sampleCalls{c,day,t,i}(index);      
                        end
                    end
                end
            end
        ...
        for d=1:D
            for i=1:R
                for c=1:C
                    ...
                    nbCalls(c,day,t,i)=nbCalls(c,day,t,i)+sampleCalls{c,day,t,i}(index);      
                end
            end
        end
    end
...
[x,fVal]=projectedGradientArmijoFeasible2(nbObservations,nbCalls,x,C,D,T,R,weight,regressor,nbLandTypes,iterMax,sigma,epsilon,indexBeta);
