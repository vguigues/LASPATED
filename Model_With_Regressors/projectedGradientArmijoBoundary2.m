...
function [x,fVal]=projectedGradientArmijoBoundary2(nbObservations,nbCalls,indexBeta,x,C,D,T,R,regressor,nbLandTypes,sizex,iterMax,sigma,betaBar,epsilon,Groups,durations,whichgroup,weight,lbounds)
...
while (k<=iterMax)
      ...
      j=0;
      while (bool==1)
       [z]=projectionRegressor(regressor,indexBeta,nbLandTypes,C,D,T,R,epsilon,sizex,x-(betaBar/(2^j))*gradient,lbounds);  
