...
function [f]=oracleObjectiveModel2(nbObservations,nbArrivals,x,C,D,T,R,regressor,nbLandTypes,Groups,weight,durations,indexBeta)
            ...
            for index1=1:length(Groups{1,m})
                ...
                for index2=1:length(Groups{1,m})
                    if (index2~=index1)
                        ...
                        for j=1:(1+nbLandTypes)
                            f=f+(weight/2)*((x(indexBeta(c,d1,t1,j))/durations(t1))-(x(indexBeta(c,d2,t2,j))/durations(t2)))^2;
                        end
                    end
                end
            end
