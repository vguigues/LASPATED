...
function [T,G,R,P,nbLandTypes,nbObservationsG,sample_calls,nbCalls,nbObservations,estimated,type,regressors,neighbors,distance]=read_calls_hol(discretization_dir)
...
if (info_file~=-1)
    ...
    R = str2num(line{1,3});
    P = str2num(line{1,4});
    nbLandTypes = str2num(line{1,5});
    nbHolidaysYear = str2num(line{1,6});
    line=strsplit(fgetl(info_file));
    nbObservationsG=zeros(1,G+1);
    for i=1:G
...
regressors=[zeros(1,R);regressors];
