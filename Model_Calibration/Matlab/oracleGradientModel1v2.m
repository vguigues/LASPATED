...
function [gradient]=oracleGradientModel1v2(nbObservations,nbArrivals,neighbors,type,distance,lambda,T,R,P,alpha,durations,Groups,whichgroup,weight)
            ...
            for p=1:P
                ...
                for j=1:length(neighbors{1,i})
                    ...
                    if (type(i)==type(neighbors{1,i}(j)))
                        ...
                        gradComponent=gradComponent+2*alpha*nbObservations(p,i,t)*nbObservations(p,j,t)*(lambda(p,i,t)-lambda(p,neighbors{1,i}(j),t))/(distance(i,neighbors{1,i}(j)));
                    end
                end
        ...
        for i=1:R
            for p=1:P
                for j=1:length(Groups{1,whichgroup(t)})
                    ...
                    if (tp~=t)
                        gradient(p,i,t)=gradient(p,i,t)+2*weight(whichgroup(t))*nbObservations(p,i,t)*nbObservations(p,i,tp)*(lambda(p,i,t)-lambda(p,i,tp));
                    end
                end
            end
        end
