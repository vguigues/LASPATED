...
function [cputime,weight,x]=crossValidationReg(sample,indexBeta,x,C,D,T,R,regressor,nbRegressors,proportion,sizex,iterMax,sigma,epsilon,nbObs,isHolidays,obsbefore,Groups,durations,whichgroup,weights)
...
for indexweights=1:length(weights)
    ...
    for indexCross=1:floor((1/proportion))
        ...
        [x,fVal]=projectedGradientArmijoFeasible2(nbObservationsCurrent,nbCallsCurrent,beta,C,D,T,R,weights(indexweights)*ones(1,nbGroups),regressor,nbRegressors,iterMax,sigma,epsilon,indexBeta,Groups,durations,sizex,whichgroup);
...
[x,fVal]=projectedGradientArmijoFeasible2(nbObservations,nbCalls,x,C,D,T,R,weight*ones(1,nbGroups),regressor,nbRegressors,iterMax,sigma,epsilon,indexBeta,Groups,durations,sizex,whichgroup);
cputime=toc;
