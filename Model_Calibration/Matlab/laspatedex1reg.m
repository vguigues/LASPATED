    ...
    for c=1:C
        for t=1:T
            for i=1:R
                ...
                % fprintf("rate %i %i %i %i = %.5f | thisnbcall = %i\n", ...
                %     c-1,day-1,t-1,i-1, rate, thisnbcall);
                % input("");
...
%         for t=1:T
%             for r=1:R
%                     fprintf('%i %i %i %i: ', c-1,d-1,t-1,r-1);
%                     for k=1:length(sample{c,d,t,r,1})
%                         fprintf('%i ', sample{c,d,t,r,1}(k));
%                     end
%                     sample(c,d,t,r)
%                     input("\n");
%             end
...
for c=1:C
    for d=1:D
        for t=1:T
            for j=1:params.nbRegressors
                theo_beta(indexBeta(c,d,t,j)) = TheoreticalBetas(c,d,t,j);
            end
        end
    end
end
params;
f = oracleObjectiveModel2(nbObservations,nbArrivals,theo_beta,C,D,T,R,regressor,params.nbRegressors,durations,indexBeta);
[lambda,obj]=laspated(model,nbObservations,nbArrivals,T,R,C,durations,Groups,whichgroup,weights,sigma,iterMax,epsilon,beta,params);
f_lam = oracleObjectiveModel2(nbObservations,nbArrivals,lambda,C,D,T,R,regressor,params.nbRegressors,durations,indexBeta);
fprintf("f(beta) = %.5f f(lam) = %.5f\n", f, f_lam);
...
%      for d=1:D
%          for t=1:4
%              estimated(1,i,compteur)=nbArrivals(1,d,t,i) / (nbObservations(1,d,t,i)*durations(t));
%              compteur=compteur+1;
%          end
%      end
%  end
