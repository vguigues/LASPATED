...
function [f]=oracleObjectiveModel1(nbObservations,nbArrivals,neighbors,type,distance,lambda,T,R,P,alpha,durations,Groups,whichgroup,weight)
            ...
            for p=1:P 
                ...
                for j=1:length(neighbors{1,i})
                    ...
                    if (type(i)==type(neighbors{1,i}(j)))
                        ...
                        f=f+(0.5*alpha)*((lambda(p,i,t)-lambda(p,neighbors{1,i}(j),t))^2)/(distance(i,neighbors{1,i}(j)));
                    end
                end
            end
        end
        ...
        for grindex=1:length(Groups)
            for j=1:length(Groups{1,grindex})
                ...
                for itp=1:length(Groups{1,grindex})
                    ...
                    if (tp~=t)
                        f=f+(0.5*weight(grindex))*((lambda(p,i,t)-lambda(p,i,tp))^2);
                    end
                end
            end
        end
