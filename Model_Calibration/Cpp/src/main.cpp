...
int main(int argc, char *argv[]) {
  ...
  // if(g_params.model == "reg"){
  // 	test_laspated_reg();
  // }else if(g_params.model == "no_reg"){
  // 	test_laspated_no_reg();
  // }else{
  // 	fmt::print("ERROR: unknown model {}. options are: reg | no_reg.\n",
  // g_params.model);
  // }

  std::string arrivals_file =
      (g_params.generator_folder == "calibration/rect" ||
       g_params.generator_folder == "calibration/hex" ||
       g_params.generator_folder == "calibration/district")
          ? "arrivals"
          : "calls";

  std::string calls_path =
      fmt::format("{}/{}.dat", g_params.generator_folder, arrivals_file);
  std::string neighbors_path =
      fmt::format("{}/neighbors.dat", g_params.generator_folder);
  std::string info_path = fmt::format("{}/info.dat", g_params.generator_folder);

  bool is_custom_data = g_params.generator_folder != "";

  // Test GeneratorRegressor;
  // if(is_custom_data){
  // 	GRBEnv env;
  ...
  // 	GeneratorRegressor gen_reg(env, calls_path, neighbors_path, info_path);
  // 	gen_reg.calibrate();
  // }else{
  // 	GRBEnv env;
    ...
    if (g_params.model == "no_reg") {
      if (is_custom_data) {
        ...
        std::string info_path =
            fmt::format("{}/info.dat", g_params.generator_folder);
        GeneratorNoRegressor gen(calls_path, neighbors_path, info_path);
        auto t0 = std::chrono::high_resolution_clock::now();
        gen.test();
        auto dt = std::chrono::high_resolution_clock::now();
        double run_time_old =
            std::chrono::duration_cast<std::chrono::nanoseconds>(dt - t0)
                .count() /
            pow(10, 9);
        fmt::print("Run time = {}\n", run_time_old);
      } else {
  ...
  // 	if(is_custom_data){
  // 		std::string calls_path = fmt::format("{}/calls.dat",
  // g_params.generator_folder); 		std::string neighbors_path =
  // fmt::format("{}/neighbors.dat", g_params.generator_folder);
  // std::string info_path = fmt::format("{}/info.dat",
  // g_params.generator_folder); 		GeneratorNoRegressor
  // gen(calls_path, neighbors_path, info_path); 		auto t0 =
  // std::chrono::high_resolution_clock::now(); 		gen.calibrate();
  // auto dt = std::chrono::high_resolution_clock::now(); 		double
  // run_time_old = std::chrono::duration_cast<std::chrono::nanoseconds>(dt -
  // t0).count() / pow(10,9); 		fmt::print("Run time = {}\n",
  // run_time_old); }else{ 		GeneratorNoRegressor gen;
  // auto t0 = std::chrono::high_resolution_clock::now();
  // gen.calibrate(); 		auto dt =
  // std::chrono::high_resolution_clock::now(); 		double
  // run_time_old = std::chrono::duration_cast<std::chrono::nanoseconds>(dt -
  // t0).count() / pow(10,9); 		fmt::print("Run time = {}\n",
  // run_time_old);
  // 	}
  // }else if(g_params.model == "no_reg" && g_params.method ==
  // "cross_validation"){ 	if(is_custom_data){ 		std::string
  // calls_path = fmt::format("{}/calls.dat", g_params.generator_folder);
  // std::string neighbors_path = fmt::format("{}/neighbors.dat",
  // g_params.generator_folder); 		std::string info_path =
  // fmt::format("{}/info.dat", g_params.generator_folder);
  // GeneratorNoRegressor gen(calls_path, neighbors_path, info_path);
  // auto t0 = std::chrono::high_resolution_clock::now();
  ...
  // / pow(10,9); 		fmt::print("Run time = {}\n", run_time_old);
  // }else{ 		GeneratorNoRegressor gen; 		double
  // proportion = g_params.cv_proportion; 		vector<double>
  // test_weights = g_params.weights_list; 		vector<double> alphas =
  // test_weights; 		auto t0 =
  // std::chrono::high_resolution_clock::now(); 		auto result =
  // gen.cross_validation(proportion, alphas, test_weights);
  // gen.write_cv_results(result); 		auto dt =
  // std::chrono::high_resolution_clock::now(); 		double
  // run_time_old = std::chrono::duration_cast<std::chrono::nanoseconds>(dt -
  // t0).count() 			/ pow(10,9); 		fmt::print("Run
  // time = {}\n", run_time_old);
  // 	}
  // }else if(g_params.model == "reg"){
  // 	GRBEnv env;
  ...
  // fmt::format("{}/neighbors.dat", g_params.generator_folder);
  // std::string info_path = fmt::format("{}/info.dat",
  // g_params.generator_folder); 		GeneratorRegressor gen(env,
  // calls_path, neighbors_path, info_path); 		auto t0 =
  // std::chrono::high_resolution_clock::now(); 		gen.calibrate();
  // auto dt = std::chrono::high_resolution_clock::now(); 		double
  // run_time_old = std::chrono::duration_cast<std::chrono::nanoseconds>(dt -
  // t0).count() / pow(10,9); 		fmt::print("Run time = {}\n",
  // run_time_old); }else{ 		GeneratorRegressor gen(env);
  // auto t0 = std::chrono::high_resolution_clock::now();
  // gen.calibrate(); 		auto dt =
  // std::chrono::high_resolution_clock::now(); 		double
  // run_time_old = std::chrono::duration_cast<std::chrono::nanoseconds>(dt -
...
void version() {
  std::cout << "Spatio Temporal Calibration of Events.\n"
            << "Version number 0.1 - Developed by: Victor Hugo "
               "(UFRJ-PESC-LABOTIM).\n";
}
...
Param load_params(int argc, char **argv) {
  ...
  generic.add_options()("version,v", "Prints version message.")(
      "help,h", "Prints help message.")("file,f",
  ...
  config.add_options()(
      ...
      "Directory with observations data. Must have a calls.dat, info.dat and "
      "neighbors.dat files. default=\"\" (use of simulated data)")(
      "method", po::value<std::string>()->default_value(""),
      ...
      "Tolerance")("sigma", po::value<double>()->default_value(0.5),
      ...
      "Proportion of data allocated to training set. Real value between 0 and "
      "1. default = 0.2")(
      ...
      "type_proj_gradient", po::value<int>()->default_value(1),
      "Type of projected gradient. 1|2 default = 1");
...
void test_laspated_reg() {
  ...
  // xt::xarray<double> lambda = laspated_reg(nb_observations, nb_arrivals,
  // regressors, lambda0); ofstream arq("x_reg.txt", std::ios::out); for(int c =
  // 0; c < C; ++c){ 	for(int d = 0; d < D; ++d){ 		for(int t = 0; t
  // < T; ++t){ 			for(int r = 0; r < R; ++r){
  // double rate = 0; 				for(int j = 0; j <
  // nb_regressors; ++j){ 					rate +=
  // lambda(c,d,t,j)*regressors(j,r);
  // 				}
  // 				arq << c << " " << d << " " << t << " " << r <<
  // " " << rate << "\n";
  // 			}
...
void test_laspated_no_reg() {
  ...
  xt::xarray<int> sample =
      xt::zeros<int>({T, R, C, static_cast<ulong>(max_obs)});
  ...
  xt::xarray<double> lambda =
      laspated_no_reg(nb_observations, nb_arrivals, durations, groups, weights,
                      alphas, distance, type_region, neighbors, lambda0);

  std::string output_file =
      (g_params.method == "calibration") ? "cv_x_no_reg.txt" : "x_no_reg.txt";
...
// double travel_time = osrm_helper.get_duration(std::make_pair(-22.900434,
// -43.277936), 	std::make_pair(-22.859857, -43.307751)); std::cout <<
// "Travel Time OSRM: " << travel_time << "\n";

// std::string calls_path = fmt::format("{}/calls.dat",
// g_params.generator_folder); std::string neighbors_path =
// fmt::format("{}/neighbors.dat", g_params.generator_folder); std::string
// info_path = fmt::format("{}/info.dat", g_params.generator_folder);
...
// gen.test();
// auto dt = std::chrono::high_resolution_clock::now();
// double run_time_old = std::chrono::duration_cast<chrono::nanoseconds>(dt -
// t0).count() 	/ pow(10,9);
...
// qr_gen.test();
// auto dt = std::chrono::high_resolution_clock::now();
// double run_time_quick = std::chrono::duration_cast<chrono::nanoseconds>(dt -
// t0).count() 	/ pow(10,9);
