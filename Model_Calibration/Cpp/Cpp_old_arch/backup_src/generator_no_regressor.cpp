...
GeneratorNoRegressor::GeneratorNoRegressor(){
	...
	for(int r = 0; r < R; ++r){
		...
		// fmt::print("Region {}: ({}, {}) blue = {}, type = {}\n", r, cx, cy, !is_red[r], type_region[r]);
		// if(is_red[r] && type_region[r] != 0){
	...
	if(read_covariates){
		...
		for(int t = 0; t < T; ++t){
			for(int r = 0; r < R; ++r){
				...
				double cy = y + 0.5;
				for(int c = 0; c < C; ++c){
					if((!is_red[r] && which_group[t] % 2 == 0) || (is_red[r] && which_group[t] % 2 != 0)){
						theoretical_lambda(t,r,c) = (constant_lambdas) ? 0.1 : (cx + cy);
					}else if((!is_red[r] && which_group[t] % 2 != 0) || (is_red[r] && which_group[t] % 2 == 0)){
						theoretical_lambda(t,r,c) = (constant_lambdas) ? 0.5  : 5*(cx + cy);
					}else{
	...
	for(int c = 0; c < C; ++c){
		for(int r = 0; r < R; ++r){
			...
			double cy = y + 0.5;
			for(int t = 0; t < T; ++t){
				estimated(c,r,t) = nb_arrivals(c,r,t) / static_cast<double>(nb_observations(c,r,t)*durations[t]);
	...
	double diff_est_theoretical = accumulate(aux.begin(), aux.end(), 0.0) / aux.size();
	aux.clear();
...
GeneratorNoRegressor::GeneratorNoRegressor(std::string calls_path, 
	...
	int test_r = 43;
	fmt::print("Testing r = {}, type = {}, neighbors = {}, distances = ", test_r, type_region[test_r], neighbors[test_r]);
	for(int s: neighbors[test_r]){
		fmt::print("{} ", distance(test_r,s));
	}
	fmt::print("\nRegressors: ");
	for(int j = 0; j < nb_regressors; ++j){
		fmt::print("{} ", regressors(j, test_r));
	}
	cin.get();
	test_r = 191;
	fmt::print("Testing r = {}, type = {}, neighbors = {}, distances = ", test_r, type_region[test_r], neighbors[test_r]);
		...
		for(int c = 0; c < C; ++c){
			for(int d = 0; d < D; ++d){
				for(int t = 0; t < T; ++t){
					...
					estimated(c,r, index) = nb_arrivals(c,r,index) / (nb_observations(c,r,index)*durations[t]);
				}
			}
		}
	}
...
void GeneratorNoRegressor::test(){
	...
	if(g_params.generator_folder == ""){
		test_weights = g_params.weights_list;
	}else{
		test_weights = {0, 0.01, 0.02, 0.03, 0.04 /*,1 ,10,100,1000,10000,100000 */};
	}
	test_weights = {0, 0.01, 0.02, 0.03, 0.04 /*,1 ,10,100,1000,10000,100000 */};
	...
	string err_arq_name = fmt::format("err_no_reg/err{}_no_reg{}_g{}_obs{}_{}{}.txt", ins_type, real, groups.size(), 
		nb_weeks, type_neighborhood, type_alpha);
	...
	for(int i = 0; i < 100; ++i){
		...
		auto f_val = projected_gradient_armijo_feasible(x);
		cv_values.push_back(average_difference(x));
	}
	double mean_cv_values = accumulate(cv_values.begin(), cv_values.end(), 0.0) / cv_values.size();
	double stdev = 0;
	for(auto val: cv_values){
		stdev += pow(val-mean_cv_values, 2);
	}
	stdev /= (cv_values.size() - 1);
	double std_error = stdev / sqrt(cv_values.size());
	fmt::print("Mean cv_values = {:.3f}, std_dev = {:.3f}, std_error = {:.3f}\n", mean_cv_values, stdev, std_error);
	...
	x_arq.close();
	string rates_arq_name = fmt::format("err_no_reg/rates_no_reg{}{}_g{}_obs{}_{}{}.txt", real, R, groups.size(), 
		nb_weeks, type_neighborhood, type_alpha);

	ofstream arq_rates(rates_arq_name, std::ios::out);
	x = epsilon*xt::ones<double>({C,R,T});
	weights = vector<double>(groups.size(), best_w);
	f_val = projected_gradient_armijo_feasible(x);
	for(int t = 0; t < T; ++t){
		for(int c = 0; c < C; ++c){
			// double sum_theoretical = 0;
			double sum_estimated = 0;
			double sum_cv = 0;
			for(int r = 0; r < R; ++r){
				// sum_theoretical += theoretical_lambda(t,r,c);
				sum_estimated += estimated(c,r,t);
				sum_cv += x(c,r,t);
			}
			arq_rates << fmt::format("{} {} {} {}\n", c, t, sum_estimated, sum_cv);
		}
	}
	arq_rates.close();
	fmt::print("rates file at {}\n", rates_arq_name);
}
...
void GeneratorNoRegressor::calibrate(){
	...
	string err_name = fmt::format("err{}_no_reg_g{}_obs{}_{}{}{}.txt", ins_type, groups.size(), 
...
void GeneratorNoRegressor::write_cv_results(CrossValidationResult& cv_result){
	...
	string type_alpha = (neighbor_factor == 0) ?  "_a0" : "";
	string holidays = (T == 4*15) ? "_h1" : "";
	string err_name = fmt::format("cv_{}_no_reg_g{}_obs{}_{}{}{}.txt", ins_type, groups.size(), 
		nb_weeks, type_neighborhood, type_alpha, holidays);
...
xt::xarray<double> GeneratorNoRegressor::oracle_gradient_model_new(xt::xarray<double>& x){
			...
			for(int t = 0; t < T; ++t){
				...
				for(int s: neighbors[r]){
					if(type_region[r] == type_region[s]){
						grad_component += 2*alpha(r,s)*nb_observations(c,r,t)*nb_observations(c,s,t)*(x(c,r,t) - x(c,s,t)) / (distance(r,s));
					}
				}
		...
		for(int r = 0; r < R; ++r){
			for(int t = 0; t < T; ++t){
				...
				for(int j = 0; j < group.size(); ++j){
					int tp = group[j];
					if(tp != t){
						gradient(c,r,t) += 2*weights[which_group[t]]*nb_observations(c,r,t)*nb_observations(c,r,tp)*(x(c,r,t)-x(c,r,tp));
					}
				}
			}
		}
	...
	for(int c = 0; c < C; ++c){
		for(int r = 0; r < R; ++r){
			for(int t = 0; t < T; ++t){
				...
				for(int s: neighbors[r]){
					if(type_region[r] == type_region[s]){
						f += (0.5*alpha(r,s))*nb_observations(c,r,t)*nb_observations(c,s,t)*pow(x(c,r,t)- x(c,s,t), 2) / distance(r,s);
					}
			...
			for(int grindex = 0; grindex < groups.size(); ++grindex){
				...
				for(int j = 0; j < group.size(); ++j){
					...
					for(int itp = 0; itp < group.size(); ++itp){
						int tp = group[itp];
						if(tp != t){
							f += (0.5*weights[grindex])*nb_observations(c,r,t)*nb_observations(c,r,tp)*(pow(x(c,r,t) - 
								x(c,r,tp), 2));
						}
					}
				}
...
CrossValidationResult GeneratorNoRegressor::cross_validation(double proportion, vector<double>& alphas, 
	...
	for(int index_alpha = 0; index_alpha < alphas.size(); ++index_alpha){
		...
		for(int index_cross = 0; index_cross < floor(1/proportion); ++index_cross){
			...
			// fmt::print("\tf test set {} = {}, first f_val = {:.1f}, last f_val = {:.1f}\n",index_cross, f, f_val[0], f_val.back());
			likelihood += f;
		}
	...
	// 	if(err < min_err){
	// 		min_err = err;
	// 		min_w = test_weights[i];
	// 		min_i = i;
	// 	}
	// }

	// x = epsilon*xt::ones<double>({C,R,T});
	// alpha = test_alphas[min_i]*neighbor_factor*xt::ones<double>({R,R});
	// weights = vector<double>(groups.size(), test_weights[min_i]);
	// auto f_val = projected_gradient_armijo_feasible(x);

	// string rates_filename = fmt::format("x_no_reg_r1_{}_{}.txt",groups.size(), nb_weeks);
	// ofstream rates_arq(rates_filename, ios::out);
	// int zone = 1;
	// for(int c = 0; c < C; ++c){
	// 	for(int t = 0; t < T; ++t){
	// 		double est = nb_arrivals(c,zone,t) / (nb_observations(c,zone,t)*durations[t]); 
	// 		rates_arq << fmt::format("{} {} {} {} {}\n", c, t, theoretical_lambda(t,zone,c), est, x(c,zone,t));
	// 	}
	// }
	// rates_arq.close();

	// string rates_filename2 = fmt::format("x_no_reg_r6_{}_{}.txt",groups.size(), nb_weeks);
	// ofstream rates_arq2(rates_filename2, ios::out);
	// zone = 6;
	// for(int c = 0; c < C; ++c){
	// 	for(int t = 0; t < T; ++t){
	// 		double est = nb_arrivals(c,zone,t) / (nb_observations(c,zone,t)*durations[t]); 
	// 		rates_arq2 << fmt::format("{} {} {} {} {}\n", c, t, theoretical_lambda(t,zone,c), est, x(c,zone,t));
	// 	}
	// }
	// rates_arq2.close();